...
//
// Sin login, LinkedIn redirige `/posts/` a la modal de login pero el overview
// (`/company/{slug}/`) muestra los últimos 2-3 posts visibles para anónimos.
// Visitamos UNA sola URL por empresa: el overview. Eso baja el costo y reduce
// la chance de bot detection.
...
try {
    ...
    log.info(`Scrapeando ${slugs.length} empresa(s) de LinkedIn (maxPosts=${maxPosts}, sin login)`)

    const bySlug = new Map()
    for (const slug of slugs) {
        bySlug.set(slug, { slug, profile: null, posts: [], errors: [] })
    }

    const requestList = slugs.map(slug => ({
    ...
    const crawler = new PlaywrightCrawler({
        ...
        preNavigationHooks: [
            async ({ request, page }) => {
                ...
                log.debug(`Esperando ${Math.round(delay)}ms antes de ${request.url}`)
        ...
        async requestHandler({ request, page, response, log: rlog }) {
            ...
            if (status === 429 || status === 999) {
                rlog.warning(`${slug}: status ${status}, retry programado`)
                throw new Error(`LinkedIn rate-limited (${status})`)
            }
            if (status >= 400) {
                if (status === 404) {
                    acc.errors.push(`404 — empresa no encontrada`)
                    return
                }
                rlog.warning(`${slug}: HTTP ${status}`)
                throw new Error(`HTTP ${status}`)
            }

            // Cerrar overlay de login modal si aparece — no bloquea pero a veces
            // captura clicks que necesitamos
            try {
                const dismissBtn = page.locator('button[aria-label="Dismiss"], button.modal__dismiss, button[data-tracking-control-name="public_profile_contextual-sign-in-modal_modal_dismiss"]').first()
            ...
            // Pequeño scroll para gatillar lazy-load de posts (si los hay)
            try {
                for (let i = 0; i < 3; i++) {
            ...
            try {
                const { profile, posts } = await extractCompanyProfile(page, slug)
                acc.profile = profile
                acc.posts   = posts.slice(0, maxPosts)
                rlog.info(`${slug}: name="${profile.name}" followers=${profile.followerCount} industry=${profile.industry ?? '—'} size=${profile.companySize ?? '—'} hq=${profile.headquarters ?? '—'} posts=${posts.length}`)
            } catch (err) {
                acc.errors.push(err.message)
                rlog.exception(err, `${slug}: extract failed`)
            }
        },
        failedRequestHandler({ request, log: rlog }, error) {
            ...
            if (slug) {
                const acc = bySlug.get(slug)
                if (acc) acc.errors.push(`falló tras retries — ${error?.message ?? 'unknown'}`)
            }
