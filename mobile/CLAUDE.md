...
### Plan v1.0 — Bloque E: infraestructura de release (EAS Update + links legales)

- **EAS Update (OTA)** — se instaló `expo-updates` (`npx expo install expo-updates`; trae `@expo/fingerprint` como transitiva, ya presente en `node_modules`). `app.json` suma `runtimeVersion: { policy: "fingerprint" }` (recalcula la runtime version sola ante cualquier cambio que afecte el código nativo — SDK upgrade, un plugin nuevo — en vez de mantenerla a mano) + `updates.url` apuntando a `https://u.expo.dev/<projectId>`. `eas.json` suma `channel` a los perfiles `preview` (`"preview"`) y `production` (`"production"`) — el perfil `development` no lleva canal (dev client, no consume OTA). **Con esto, un fix de solo JS/estilos ya no necesita un build nuevo de Play Store**: `eas update --branch production --channel production --message "..."` publica el bundle y los dispositivos con la app instalada lo bajan solos la próxima vez que abren la app. **Importante:** esta capacidad recién existe a partir del **próximo** build nativo — el `.aab` que ya se subió a Internal Testing (Bloques A+B) se compiló antes de este cambio y no tiene `expo-updates` embebido, así que no puede recibir updates OTA; el próximo build (Bloques C+D+E) sí.
- **Links legales** — fila "Términos de servicio" / "Política de privacidad" en `ProfileScreen`, debajo de "Apariencia" y antes de "Cerrar sesión", cada una `Linking.openURL` a `https://blisstracker.app/condiciones` / `/privacidad` (mismos documentos que sirve la web en `TermsPage.jsx`, no hay contenido propio de mobile). Exigencia de Play Store: la política de privacidad tiene que ser accesible desde dentro de la app, no solo en la ficha de Play Console.
- **Versionado** — ya estaba resuelto de antes (`eas.json` → `cli.appVersionSource: "remote"`, perfil `production` con `autoIncrement: true`), sin cambios en este bloque.
- **Crash reporting (Sentry) — deliberadamente NO incluido en v1.0.** Requiere que el usuario cree una cuenta de Sentry (decisión suya, no algo que se pueda resolver desde el código) y correr `npx @sentry/wizard`. Queda anotado como el primer candidato de v1.1 si hace falta diagnosticar crashes reportados por usuarios reales.

Verificado con `npx expo export --platform android` (1102 módulos, sin errores) y `npx expo-doctor` (20/21 — mismo desalineamiento de versiones de patch preexistente de antes de este cambio, no introducido por `expo-updates`). Sin carpetas `android`/`ios` generadas (sigue 100% managed workflow). **Pendiente**: el próximo build de EAS confirma que `expo-updates` quedó bien enlazado (revisar que no rompa el arranque) y, ya con ese build instalado, probar que `eas update` efectivamente empuja un cambio sin pasar por Play Store.
