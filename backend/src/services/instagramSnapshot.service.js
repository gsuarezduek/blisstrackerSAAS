...
async function saveInstagramSnapshot(projectId, workspaceId, month, preloadedMetrics = null) {
  ...
  if (!metrics) {
    ...
    if (integration.scopes === 'scrape') {
      metrics = await scrapeInstagramProfile(integration.propertyId, { targetMonth: month, workspaceId, projectId, action: 'monthly_snapshot', actionLabel: 'Instagram — snapshot mensual' })
    } else {
