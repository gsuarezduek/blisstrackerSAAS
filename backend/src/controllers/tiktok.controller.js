...
/**
 * DELETE /api/marketing/projects/:id/tiktok/snapshots/:month
...
async function deleteSnapshot(req, res, next) {
  try {
    ...
    const { count } = await prisma.tikTokSnapshot.deleteMany({ where: { projectId, workspaceId, month } })
    if (count === 0) return res.status(404).json({ error: 'Snapshot no encontrado' })

    await prisma.tikTokFollowerLog.deleteMany({ where: { projectId, workspaceId, date: { startsWith: month } } })
