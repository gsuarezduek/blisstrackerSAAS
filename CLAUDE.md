...
**Botón "+" reutiliza Feedback, no lo duplica:** `FeedbackButton.jsx` se dividió en `FeedbackModal.jsx` (props `open`/`onClose`, el formulario en sí) + `FeedbackButton.jsx` (el FAB flotante, sin cambio de comportamiento). El menú "+" del input del chat abre el mismo `FeedbackModal` para "Nueva sugerencia / Reportar error", pegando al `POST /api/feedback` de siempre — el chat no tiene su propio sistema de reportes.

**Deep link desde un proyecto:** `ProjectDetail.jsx` tiene una entrada "💬 Chat" que navega a `/chat/<slug del canal del proyecto>` (el slug viaja en `Project.chatChannel.slug`, incluido en `includeDetails` de `projects.controller.js` — mismo objeto que ya devuelve `GET /api/projects`). Al entrar a `/chat/:slug` ese canal queda seleccionado por defecto.
...
/chat             → Chat.jsx             (PrivateRoute) — /chat/:slug? abre directamente ese canal (usado por el deep link desde ProjectDetail); sin slug abre #general o el último canal visitado
