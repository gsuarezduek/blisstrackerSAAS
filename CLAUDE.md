...
**Atajos de teclado globales:** `GlobalShortcuts.jsx` se monta una sola vez en `App.jsx` (dentro de `<BrowserRouter>`, después de `<Routes>`) y maneja atajos para todo usuario autenticado en un subdominio de workspace (`enabled = !!user && isWorkspaceSubdomain()`). Atajos: `N` → nueva tarea (renderiza un `AddTaskModal` global, sin proyecto bloqueado), `G` luego `D`/`P`/`R` → navega a Dashboard / Mis Proyectos / Tiempo real (acorde estilo Gmail, ventana de 1.2s), `?` → overlay de ayuda con el catálogo de atajos, `Esc` → cierra la ventana abierta. El catálogo de ayuda también lista `Ctrl/Cmd+B` (abrir/cerrar la pizarra de notas), pero ese atajo lo maneja `NotesBoard.jsx` por su cuenta (no `GlobalShortcuts`, que ignora combinaciones con modificadores) — figura en la lista solo como documentación. Las teclas de una sola letra **no** se disparan mientras el foco está en un campo editable (`input`/`textarea`/`select`/contenteditable) ni con modificadores (Ctrl/Cmd/Alt). Cuando se crea una tarea desde el modal global, `GlobalShortcuts` emite el evento `window` `bliss:task-created` (detalle = la tarea); `Dashboard.jsx` lo escucha y recarga con `loadToday()` (el botón "+ Agregar tarea" del propio Dashboard usa su modal local y **no** emite el evento, así no hay doble refresco).
