...
**`ChannelSwitcher.jsx` — buscador + orden por prioridad (sin duplicar canales):** buscador que filtra en vivo por nombre (case-insensitive, `channel.name`). El listado NO se agrupa solo por `kind` — se reordena por prioridad y cada canal aparece **una sola vez**, en el grupo de mayor prioridad al que califica: **Menciones** (`mentionCount > 0`, label rojo, cualquier `kind`) → **Destacados** (`starred`, reutiliza el mismo `ProjectStar` de "Mis Proyectos" — un canal de proyecto es favorito si su proyecto lo es, sin concepto de favorito propio del chat ni botón para togglearlo desde acá) → `#general` (sin label) → **Proyectos** → **Canales**. Dentro de los grupos **Proyectos** y **Canales** (no en Menciones/Destacados) el orden es **alfabético** (`localeCompare` con sensibilidad `base`, ignora mayúsculas/acentos). Los grupos vacíos no se renderizan; el buscador filtra antes de agrupar, así un grupo sin matches simplemente desaparece. `chat.controller.js` `listChannels` agrega el campo `starred` por canal consultando `ProjectStar` del usuario actual (solo para canales `kind='project'`, `projectId` no nulo).
