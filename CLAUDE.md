...
- **Métricas**: `Hechas` (tareas completadas en el período **por fecha de completado** — `fetchRows` adjunta `completedDate` en la TZ del workspace, así cuenta lo terminado aunque la tarea se haya arrastrado de días previos), `Horas` (tiempo activo de esas tareas: completado−iniciado−pausas, tope 8h, o `minutesOverride`), `Tasa` (completadas÷creadas: ritmo de cierre vs creación; >100% = bajando backlog), `Δ tareas/horas` (cambio del ritmo **por día trabajado** vs período previo, para neutralizar días no trabajados), `atascadas` (PAUSED/BLOCKED sin moverse >7d), `Estado` (semáforo determinístico `memberStatus` en el servicio: `inactive`/`down`/`stuck`/`up`/`ok`/`nodata`).
- **Asistencia** (`getAttendanceStats`): por persona en el período actual — `businessDays` (lun-vie), `daysPresent` (días hábiles con WorkDay), `leaveDays` (días hábiles de `VacationRequest` aprobadas, recortados al período), `expectedDays` (= hábiles − licencias), `absentDays` (= esperados − presentes), `lateDays` (tardanzas: primer login del día vs `workStartTime` + tolerancia; `null` sin horario). Separa "el mes está a medias / hubo licencia" de "faltó". Se muestra como columna compacta (`presentes/esperados` + badge `⏰N`) y bloque detallado en la fila expandida.
- **Comparación con el equipo**: `computeBenchmark` devuelve la **mediana** (no promedio) de `completed`/`horas`/`tasaCompletado`/`tareasPorDia` de quienes tuvieron actividad; el front muestra indicadores `▲/▼ vs equipo` por celda, un bloque comparativo en la expansión y la mediana en el pie.
- **UI** (`ProductivityTab.jsx`): panel de ayuda colapsable (glosario "¿Cómo se lee esta sección?"), resumen ejecutivo con chips clickeables que filtran por estado (en baja / inactivos / con atascos / en alza), y **barras de horas activas por semana** (reemplazan el viejo sparkline de tasa). La vista por proyecto sigue en `ProductivityByProjectTab.jsx`.
- **Aviso semanal por mail** (`productivityDigest.service.js`): cron lunes 08:00 ART (`sendAllProductivityDigests`) que, por cada workspace activo con `Workspace.productivityDigestEnabled` (default `true`), arma el digest del mes en curso y envía a los **admins/owners** un mail con las personas en alerta (`inactive`/`down`/`stuck`). **Solo envía si hay ≥1 persona en alerta** (cero ruido). Email `sendProductivityDigestEmail` (type `productivityDigest` en `EmailLog`). Toggle en Preferencias → Globales ("Aviso semanal de Productividad").
...
- **MiniDashboard** — active users, avg tenure, incomplete profiles, upcoming birthdays/anniversaries (30d), role distribution, last login per person, **puntualidad del equipo + tardanzas de hoy** (slides condicionales, solo si hay horarios configurados — ver "Horario laboral y tardanzas"). Las **tarjetas pequeñas con métrica mensual son clickeables** → abren `MetricHistoryModal` con la evolución mes a mes: Personas activas, Antigüedad promedio, Proyectos por persona, Horario promedio de ingreso y Puntualidad del equipo (ver "Snapshots de métricas de RRHH"). Las tarjetas "de hoy" (Iniciaron sesión hoy, Llegaron tarde hoy) y "Legajos completos" **no** tienen historial (métricas puntuales del día / condicionales).
...
/marketing        → Marketing.jsx        (PrivateRoute) — tabs: GEO, Web, SEO, Keywords, Canibalización, Instagram, TikTok, LinkedIn (próximamente), Competidores, Meta Ads, Google Ads, Salud, Informes. Sub-tabs con `soon: true` (LinkedIn, YouTube) muestran `ComingSoon` aunque el código exista.
...
| `15 6 1 * *` (1° mes 06:15) | ART | Guarda snapshot de competidores de Instagram (scraping) del mes anterior |
| `30 6 1 * *` (1° mes 06:30) | ART | Guarda snapshots de métricas de RRHH (proyectos/persona, antigüedad) del mes anterior por workspace |
