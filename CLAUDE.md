...
| `0 1 1 * *` (1° mes 01:00) | ART | **Cadena mensual de snapshots** (`MONTHLY_CHAIN`, secuencial): GEO → GA4 → GSC+DomainRating → PageSpeed → keywords → Instagram → TikTok → YouTube → LinkedIn → Facebook → Ads → competidores → métricas RRHH. Todos del mes anterior. Cada job aislado en su try/catch; un fallo no corta la cadena. |
