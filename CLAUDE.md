...
- **UI** (`ProductivityTab.jsx`): el fetch de `/admin/productivity` vive en el componente **raíz** (así el encabezado "Δ horas del equipo" se ve en ambas sub-pestañas sin doble request). Tabla por persona en orden `Δ horas · Tareas · Horas · Tasa · Asistencia · Estado`, con panel de ayuda colapsable, chips de estado que filtran, y en la fila expandida un **gráfico de líneas de horas por semana de las últimas 12 semanas** (`getHoursHistory`, **independiente del período** — contexto de tendencia) + tiempo por proyecto + Horas y Asistencia + comparación con el equipo + análisis IA. La vista por proyecto sigue en `ProductivityByProjectTab.jsx`.
