...
- **Métricas**: `Hechas` (completadas con día en el período), `Horas` (tiempo activo de esas tareas: completado−iniciado−pausas, tope 8h, o `minutesOverride`), `Tasa` (completadas÷creadas), `Δ tareas/horas` (cambio del ritmo **por día trabajado** vs período previo, para neutralizar días no trabajados), `atascadas` (PAUSED/BLOCKED sin moverse >7d), `Estado` (semáforo determinístico con umbrales en el controller: `inactive`/`down`/`stuck`/`up`/`ok`/`nodata`).
