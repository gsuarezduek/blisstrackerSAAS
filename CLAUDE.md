...
**Reuniones del proyecto (ficha del proyecto):** Registro de reuniones por proyecto (modelos `ProjectMeeting` + `ProjectMeetingTodo` + `ProjectMeetingParticipant`), con formato similar a la reunión L10 de EOS Tracción pero **sin puntaje** y **con cronómetro de duración que cuenta tiempo a los participantes**. Cada `ProjectMeeting` es un evento con `date` (`YYYY-MM-DD`), `type` (`internal` = equipo | `client` = cliente), `notes` (HTML del editor de notas libres), y un cronómetro: `startedAt`/`endedAt`/`durationMins`. Cada reunión tiene `ProjectMeetingTodo[]` (tareas con responsable) y `ProjectMeetingParticipant[]` (asistentes).
