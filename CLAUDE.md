...
**Recordatorios de próxima acción (email diario):** `salesReminders.service.js` `sendAllSalesReminders()` — cron diario **08:00 ART** (`0 8 * * *`). Por cada workspace con el flag `ventas` habilitado (respeta opt-out), agrupa los leads con `nextActionDueAt` **para hoy** o **vencida** (estado no terminal) por responsable efectivo (`nextActionOwnerId` || `ownerId`) y le manda un email (`sendSalesReminderEmail`, type `salesReminder`) con las acciones de hoy + vencidas, cada una con link a `…/ventas?lead=<id>`. **Dedup**: a lo sumo 1 envío por persona por día (consulta `EmailLog`). Solo envía si el responsable tiene ≥1 acción pendiente.
...
PUT    /api/ventas/leads/:id/next-action           # { title, dueAt?, ownerId? } | { title:null } limpia
