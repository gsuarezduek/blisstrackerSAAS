...
function CrossProjectMetaAdsPanel({ onSelectProject }) {
  ...
  useEffect(() => {
    ...
    const req = period === 'live'
      ? api.get('/marketing/summary/ads-live?type=meta_ads').then(r => r.data.results)
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5">
      <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">
          Meta Ads por proyecto {data && `(${rows.length})`} <span className="font-normal text-gray-400">· {period === 'live' ? 'este mes, en vivo' : 'último snapshot cerrado'}</span>
        </h3>
