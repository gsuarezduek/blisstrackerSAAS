...
function CrossProjectMetaAdsPanel({ onSelectProject }) {
  ...
  useEffect(() => {
    api.get('/marketing/summary/ads?type=meta_ads')
  ...
  if (!data?.length) return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-10 text-center">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">Todavía no hay snapshots de Meta Ads. Los snapshots se guardan el primer día de cada mes, o podés guardar uno manualmente desde dentro del proyecto.</p>
    </div>
  )
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5">
      <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">
        Meta Ads por proyecto ({data.length}) <span className="font-normal text-gray-400">· último snapshot disponible</span>
      </h3>
          ...
          <div key={p.projectId} className="flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5">
                <div className="h-1.5 rounded-full bg-blue-500" style={{ width: `${Math.round((p.spend / maxSpend) * 100)}%` }} />
              </div>
